...
const transferAmount = catchAsync(async (req, res) => {
    ...
    if(!userReceive) {
        return res.status(404).json({
            status: 'error',
            message: 'This user account to which you want to send the amount does not exist.'
        })
    }
    ...
    if(userReceiveId === userTransferId) {
        return res.status(400).json({
            status: 'error',
            message: 'The user to whom you are sending is the same user who is sending'
        })
    }
    ...
    if(amount > userTransfer.amount) {
        return res.status(404).json({
            status: 'error',
            message: 'The user cannot transfer more than the amount he has in his account.'
        })
    }
